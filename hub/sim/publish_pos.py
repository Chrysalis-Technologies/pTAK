...
def main() -> int:
    parser = argparse.ArgumentParser(description="Publish simulated position message to Meshtastic MQTT topic.")
    parser.add_argument("--node-id", required=True, help="Node identifier used in topic path")
    parser.add_argument("--lat", type=float, default=43.04531)
    parser.add_argument("--lon", type=float, default=-76.12270)
    parser.add_argument("--speed", type=float, default=3.4)
    parser.add_argument("--course", type=float, default=182.0)
    ...
    topic = f"meshtastic/{args.node_id}/position"
    payload = {
    ...
    client.disconnect()
    print(f"Published position message to {topic}")
    return 0
