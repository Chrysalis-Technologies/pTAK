...
Write-Host ("Waiting for FTS API on {0}:{1} ..." -f $hostName, $apiPort)
do {
...
try {
  $uri = "http://{0}:{1}" -f $hostName, $apiPort
  $r = Invoke-WebRequest -UseBasicParsing -Uri $uri -Method GET -TimeoutSec 5
