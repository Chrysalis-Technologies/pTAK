...
$payload = [ordered]@{
  timestamp = $timestamp
  callsign  = $NodeId
  lat       = [double][Math]::Round([double]43.04531, 6)
  lon       = [double][Math]::Round([double]-76.12270, 6)
  speed     = [double]3.4
  course    = [double]182.0
}

$json = $payload | ConvertTo-Json -Compress
$topic = "meshtastic/$NodeId/position"

Write-Host "Publishing position to $topic" -ForegroundColor Cyan
...
Write-Host "Published position payload." -ForegroundColor Green
