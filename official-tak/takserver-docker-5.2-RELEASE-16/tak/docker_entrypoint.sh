...
# Ensure helper scripts are available even when the data directory was pre-populated
for helper in cert-metadata.sh config.cfg generateClusterCerts.sh generateClusterCertsConfigMap.sh generateClusterCertsIfNoneExist.sh makeCert.sh makeRootCa.sh revokeCert.sh; do
	if [[ -f "${TR}/certs.orig/${helper}" && ! -f "${CR}/${helper}" ]]; then
		cp "${TR}/certs.orig/${helper}" "${CR}/"
	fi
done
chmod +x ${CR}/*.sh
if [[ ! -f "${CR}/crl_index.txt" ]]; then
	touch "${CR}/crl_index.txt"
	printf 'unique_subject = no\n' > "${CR}/crl_index.txt.attr"
fi
...
if [[ $? -ne 0 ]]; then
	echo "WARNING: Failed to grant admin role to ${ADMIN_CERT_NAME}. Rerun certmod manually once the server is ready."
fi
set -e
